...
class Eos:
    ...
    def __init__(self, config):
        self.http_address = (
            config["http_address"]
            if "http_address" in config
            else "http://127.0.0.1:8888"
        )
        self.key_provider = (
            config["key_provider"]
            if "key_provider" in config
            else "5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3"
        )
    ...
    def push_transaction(self, acct_contract, func_name, acct_owner, permission, data):
        ...
        response = muterun_js(
            self.current_dir + "/js/PushContractTransaction.js", arguments=arguments
        )
        if response.exitcode == 0:
            print(response.stdout.decode("utf8"))
        else:
